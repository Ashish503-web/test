            ...
            <div class="account-head">
                ...
                @if ($order->canCancel())
                    <span class="account-action">
                        <a href="{{ route('customer.orders.cancel', $order->id) }}"
                           class="theme-btn light unset pull-right"
                ...
                @if($reorder->value == 1)
                    ...
                    @foreach($order->items as $item)
                        ...
                        <form action="{{ route('cart.reorder', $item->product_id) }}" method="POST">
                            @csrf
                            <input type="hidden" name="product_id" value="{{ $item->product_id }}">
                            <input type="hidden" name="quantity" value="{{$item->qty_ordered}}">

                            @if($in_stock_product === 0)
                                <button class="btn btn-lg btn-primary addtocart disabled">
                                    Re-order
                                </button>
                            @else
                                <button class="btn btn-lg btn-primary addtocart"
                                        onclick="return confirm('Do you really want to perform this action?')">
                                    Re-order
                                </button>
                            @endif
                ...
                {{------------------------ end  re-order functionality page------------}}
